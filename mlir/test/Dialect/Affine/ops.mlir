...
func.func @affine.yield() {
  ...
  // CHECK-NEXT: }
  //
  // GENERIC:      "affine.for"() <{dependentTensorIterArgRefinements = [], dependentTensorResultRefinements = [], lowerBoundMap = #[[$map]], operandSegmentSizes = array<i32: 0, 0, 0>, step = 1 : index, upperBoundMap = #[[$map1]]}> ({
  // GENERIC-NEXT: ^bb0(%{{.*}}: index):
  // GENERIC-NEXT:   "affine.yield"() : () -> ()
  // GENERIC-NEXT: }) : () -> ()
  affine.for %i = 0 to 10 {
