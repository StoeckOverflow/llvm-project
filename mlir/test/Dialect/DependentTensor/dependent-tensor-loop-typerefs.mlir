...
// CHECK: scf.for {{.*}} iter_args(%[[ARG:.*]] = {{.*}}) -> (tensor<?xf32>) #types[%[[ARG]] : #tensor<[%{{.*}}], f32> -> #tensor<[%{{.*}}], f32>] #region_types[%[[ARG]] : #tensor<[%{{.*}}], f32> -> #tensor<[%{{.*}}], f32>] {
...
func.func @split_multiple_scf_loop_type_refs(%m : index, %n : index)
  ...
  %r0, %r1 = scf.for %i = %c0 to %m step %c1
      ...
      #types[%a : #tensor<[%m], f32> -> #tensor<[%m], f32>,
             %b : #tensor<[%n], f32> -> #tensor<[%n], f32>]
      #region_types[%a : #tensor<[%m], f32> -> #tensor<[%m], f32>,
                    %b : #tensor<[%n], f32> -> #tensor<[%n], f32>] {
...
// CHECK: scf.for {{.*}} iter_args(%[[A:.*]] = {{.*}}, %[[B:.*]] = {{.*}}) -> (tensor<?xf32>, tensor<?xf32>) #types[%[[A]] : #tensor<[%{{.*}}], f32> -> #tensor<[%{{.*}}], f32>, %[[B]] : #tensor<[%{{.*}}], f32> -> #tensor<[%{{.*}}], f32>] #region_types[%[[A]] : #tensor<[%{{.*}}], f32> -> #tensor<[%{{.*}}], f32>, %[[B]] : #tensor<[%{{.*}}], f32> -> #tensor<[%{{.*}}], f32>] {
...
func.func @distinct_actual_and_region_refs(%actual_in : index,
  ...
  %r = scf.for %i = %c0 to %actual_in step %c1 iter_args(%arg = %init)
      ...
      #region_types[%arg : #tensor<[%region_in], f32> -> #tensor<[%region_out], f32>] {
...
// CHECK: affine.for {{.*}} iter_args(%[[ARG:.*]] = {{.*}}) -> (tensor<?xf32>) #types[%[[ARG]] : #tensor<[%{{.*}}], f32> -> #tensor<[%{{.*}}], f32>] #region_types[%[[ARG]] : #tensor<[%{{.*}}], f32> -> #tensor<[%{{.*}}], f32>] {

// -----

func.func @body_local_yield_ref_roundtrip(%m : index) -> tensor<?xf32>
    #types[] -> #tensor<[%m], f32> {
  %c0 = arith.constant 0 : index
  %c1 = arith.constant 1 : index
  %init = dependent_tensor.make () #tensor<[%m], f32> : tensor<?xf32>
...
// CHECK-LABEL: func.func @body_local_yield_ref_roundtrip
// CHECK: scf.for {{.*}} #types[%{{.*}} : #tensor<[%{{.*}}], f32> -> #tensor<[%{{.*}}], f32>] {
// CHECK-NOT: #region_types
...
func.func @duplicate_loop_type_ref(%m : index, %t : tensor<?xf32>) {
  ...
  %r = scf.for %i = %c0 to %m step %c1 iter_args(%arg = %t)
      -> (tensor<?xf32>)
      // expected-error@+2 {{duplicate dependent tensor loop boundary value}}
      #types[%arg : #tensor<[%m], f32> -> #tensor<[%m], f32>,
             %arg : #tensor<[%m], f32> -> #tensor<[%m], f32>] {
    scf.yield %arg : tensor<?xf32>
  }
  return
}
...
func.func @duplicate_region_type_ref(%m : index, %t : tensor<?xf32>) {
  %c0 = arith.constant 0 : index
  %c1 = arith.constant 1 : index
  %r = scf.for %i = %c0 to %m step %c1 iter_args(%arg = %t)
      -> (tensor<?xf32>)
...
func.func @old_loop_type_syntax_rejected(%m : index, %t : tensor<?xf32>) {
  ...
  %r = scf.for %i = %c0 to %m step %c1 iter_args(%arg = %t)
      -> (tensor<?xf32>)
      // expected-error@+1 {{expected inline dependent tensor loop result refs}}
      #types[%arg : #tensor<[%m], f32>] -> #tensor<[%m], f32> {
    scf.yield %arg : tensor<?xf32>
  }
  return
