...
// CHECK: %[[Y_FINAL:.*]] = scf.for %[[N_IDX:.*]] = %{{.*}} to %[[N]] step %{{.*}} iter_args(%[[YN:.*]] = %[[Y]]) -> (tensor<?x?x?x?xf32>) #types[%[[YN]] : #tensor<[%[[N]], %[[COUT]], %[[OH]], %[[OW]]], f32> -> #tensor<[%[[N]], %[[COUT]], %[[OH]], %[[OW]]], f32>] #region_types[%[[YN]] : #tensor<[%[[N]], %[[COUT]], %[[OH]], %[[OW]]], f32> -> #tensor<[%[[N]], %[[COUT]], %[[OH]], %[[OW]]], f32>] {
// CHECK:   %[[Y_CO:.*]] = scf.for %[[CO:.*]] = %{{.*}} to %[[COUT]] step %{{.*}} iter_args(%[[YCO:.*]] = %[[YN]]) -> (tensor<?x?x?x?xf32>) #types[%[[YCO]] : #tensor<[%[[N]], %[[COUT]], %[[OH]], %[[OW]]], f32> -> #tensor<[%[[N]], %[[COUT]], %[[OH]], %[[OW]]], f32>] #region_types[%[[YCO]] : #tensor<[%[[N]], %[[COUT]], %[[OH]], %[[OW]]], f32> -> #tensor<[%[[N]], %[[COUT]], %[[OH]], %[[OW]]], f32>] {
// CHECK:     %[[Y_OH:.*]] = scf.for %[[OH_IDX:.*]] = %{{.*}} to %[[OH]] step %{{.*}} iter_args(%[[YOH:.*]] = %[[YCO]]) -> (tensor<?x?x?x?xf32>) #types[%[[YOH]] : #tensor<[%[[N]], %[[COUT]], %[[OH]], %[[OW]]], f32> -> #tensor<[%[[N]], %[[COUT]], %[[OH]], %[[OW]]], f32>] #region_types[%[[YOH]] : #tensor<[%[[N]], %[[COUT]], %[[OH]], %[[OW]]], f32> -> #tensor<[%[[N]], %[[COUT]], %[[OH]], %[[OW]]], f32>] {
// CHECK:       %[[Y_OW:.*]] = scf.for %[[OW_IDX:.*]] = %{{.*}} to %[[OW]] step %{{.*}} iter_args(%[[YOW:.*]] = %[[YOH]]) -> (tensor<?x?x?x?xf32>) #types[%[[YOW]] : #tensor<[%[[N]], %[[COUT]], %[[OH]], %[[OW]]], f32> -> #tensor<[%[[N]], %[[COUT]], %[[OH]], %[[OW]]], f32>] #region_types[%[[YOW]] : #tensor<[%[[N]], %[[COUT]], %[[OH]], %[[OW]]], f32> -> #tensor<[%[[N]], %[[COUT]], %[[OH]], %[[OW]]], f32>] {
...
func.func @dependent_conv2d_kernel_inner_yield_mismatch(
  ...
  %Y_final = scf.for %n_idx = %c0 to %n step %c1 iter_args(%Yn = %Y)
      ...
      #types[%Yn : #tensor<[%n, %cout, %oh, %ow], f32> -> #tensor<[%n, %cout, %oh, %ow], f32>]
      #region_types[%Yn : #tensor<[%n, %cout, %oh, %ow], f32> -> #tensor<[%n, %cout, %oh, %ow], f32>] {
    // expected-error@+1 {{'scf.for' op loop region yield type reference does not match yielded refinements}}
...
func.func @dependent_conv2d_kernel_loop_result_metadata_mismatch(
  ...
  %Y_final = scf.for %n_idx = %c0 to %n step %c1 iter_args(%Yn = %Y)
      ...
      #region_types[%Yn : #tensor<[%n, %cout, %oh, %ow], f32> -> #tensor<[%n, %cout, %ow, %oh], f32>] {
