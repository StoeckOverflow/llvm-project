  ...
  scf.for %i = %c0 to %n step %c1 {
    scf.for %j = %c0 to %m step %c1 {
      %sum = scf.for %kk = %c0 to %k step %c1 iter_args(%acc = %zero) -> (f32) {
        %a = dependent_memref.load %A[%i, %kk] #memref<[%n, %k], f32, offset: 0, strides: [%as0, %as1]> : memref<?x?xf32>
        %b = dependent_memref.load %B[%kk, %j] #memref<[%k, %m], f32, offset: 0, strides: [%bs0, %bs1]> : memref<?x?xf32>
