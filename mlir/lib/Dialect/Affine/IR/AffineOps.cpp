...
static FailureOr<DependentTensorTypeRef>
getAffineTypeRefFromValue(Value value, Operation *op) {
...
getAffineDependentTensorRefinementFromBlockArgument(BlockArgument arg) {
  ...
  return iface.getDependentTensorBlockArgumentRefinement(
...
getAffineDependentTensorRefinementFromValue(Value value) {
  if (auto result = dyn_cast<OpResult>(value)) {
    ...
    if (!iface)
      return failure();
    return iface.getDependentTensorResultRefinement(result.getResultNumber());
  }
  return getAffineDependentTensorRefinementFromBlockArgument(
      cast<BlockArgument>(value));
}

static ParseResult populateAffineDependentTensorLoopTypeRefsFromInits(
...
populateAffineDependentTensorLoopTypeRefs(AffineForOp forOp) {
  ...
  ValueRange yieldedValues = yield ? yield.getOperands() : ValueRange();
  dependent_tensor::inferMissingLoopTypeRefs(
      forOp.getResultTypes(), forOp.getInits(), forOp.getRegionIterArgs(),
      yieldedValues,
...
AffineForOp::getDependentTensorLoopOperandTypeRef(unsigned operandNumber) {
...
FailureOr<DependentTensorTypeRef>
AffineForOp::getDependentTensorLoopResultTypeRef(unsigned resultNumber) {
  if (const DependentTensorLoopTypeRef *ref = dependent_tensor::findLoopTypeRef(
